...
- ✅ **#2 закрыт:** блок «ГЛУБИНА ПОД ФАЗУ И ПОЛ» в схеме `/day-plan` — пери (лён/соя/крестоцветные+магний-сон/D3+K2/витекс), зрелый этап (белок+кость), мужские (цинк/жиры/ашваганда/CoQ10/аргинин). Live-тест: жен-пери и муж-46 дают разные памятки.
- ✅ **#3 закрыт:** граммовка порций масштабируется под вес в каждом варианте (жен-55кг→рыба 150г/~28г белка; муж-95кг→лосось 150–180г/~30–38г белка).
- ✅ **Правило гормонов (часть #6):** слово «гормон/гормональный фон/баланс» в VIO/PRO ЗАПРЕЩЕНО → «внутренний баланс». Утечка в нутрицевтиках `/day-plan` устранена на 3 слоях: KB `[P-F2]`, цель эвфемизма в `_FEED_DEMED` (перецелена на «внутренний баланс» + catch-all «гормональн- фон/баланс/характер»), санитайзер ВЫХОДА `/day-plan` (`deMedicalizeFeed` по items — раньше выход не чистился). `WELLNESS_STYLE_OURA` — тариф-зависимая. Live: 0 утечек в `/day-plan` и `/analyze`. См. память `feedback-wellness-terminology-mandatory`.

- ✅ **#5 закрыт:** фолбэк памятки локализован `L({ru,en})` — `_bpChapters` (заголовки видны ВСЕГДА, даже при ИИ), весь `_bpChapterData`, обёртка PDF `downloadDayPlan`. Английский = универсальный фолбэк для всех 12 языков (продукт глобальный, EN основной, RU только владелец↔ассистент — память `feedback-english-primary-global`). Проверено: EN-фолбэк 0 кириллицы.
- ✅ **#6 закрыт** (см. правило гормонов выше — паритет VIO/PRO проверен live).

## 🔴 Осталось
4. **Native-напоминалки** по главам (Capacitor local notifications) + сохранение PDF/картинки в Файлы. ⚠️ бОльшая native-задача (`app/`), нужен вектор владельца (когда напоминать, что именно).
7. **Визуальный проход владельца** по живой странице (curl-дифф жен-пери vs муж-46 уже подтверждён: памятки различаются, 0 утечек).
