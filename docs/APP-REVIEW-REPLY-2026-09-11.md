...
> приложения (воркер уже задеплоен). Пункт 2 **не сделан**: политика — юр-текст на 12 языках, и в
> ней есть противоречие («no name or email»), а «My guide» просит почту и имя. Переписать
> отдельно до отправки ответа Apple. Пункт 3 ждёт решения владельца.
