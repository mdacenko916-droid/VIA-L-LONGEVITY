...
# Ссылка «← Назад в приложение» на юр-страницах ведёт в ../index.html, то есть в приложение с
# нуля — а оно загружается на вкладке «Сегодня». Человек, зашедший в политику ИЗ ПРОФИЛЯ,
# оказывался не там, откуда пришёл (живой Android 2026-09-01). В бандле дописываем ?tab=card:
# приложение читает этот параметр и открывает профиль. Сайт не трогаем — правка только в копии.
# Плюс history.back(), когда история есть: с ЭКРАНА ОПЛАТЫ (Terms/Privacy на пейволле) жёсткий
# ?tab=card уводил в профиль — человек не понимал, куда делся экран подписки (2026-09-11).
# Возврат по истории отдаёт ровно тот экран, с которого ушли; ?tab=card остаётся запасным.
sed -i '' -e 's#href="\.\./index\.html"#href="../index.html?tab=card" onclick="if(history.length>1){history.back();return false;}"#g' "$WWW"/legal/*.html
