...
- [~] **IAP для VIA·L** (месячная подписка, цена из стора — RevenueCat, см. `IAP.md`) — код готов, ждёт настройки владельцем
...
- **PRO в приложении:** РЕШЕНО (2026-07-15) — только Apple IAP (auto-renewable subscription,
  месячная подписка), НЕ код с сайта/Hotmart (Hotmart как канал продаж отвязан). Код готов — см. `IAP.md`
