...
- **2026-09-24** — **Памятка дня собирается движком у всех; пробник не обнуляется переустановкой.**
  (1) **Движок памятки включён всем** (`DAYPLAN_ENGINE=on`, секрет воркера). Выключатель стоял с 22.09,
  но в режиме «по устройствам» не совпадал ни с одним живым проходом: в `ai_usage` были только наши
  `engine-test`, а реальные памятки 23 и 24.09 делала модель ($0.0855 / $0.0576). Воркер помечает план
  движка полем `engine`; обе страницы по метке пишут «Собрано по вашим данным: пол, фаза, вес и сон»
  (12 языков) вместо «Персонализировано ИИ» (`9c8b9d6`). Пробник памятку у ИИ не заказывает — обещание
  «меню с фото при подписке» не нарушено. (2) **Постоянный номер устройства `dev`** (`2a045df`, ждёт
  сборки): iOS — связка ключей (`VialDeviceId.swift`, переживает удаление приложения), Android — SSAID
  (`@capacitor/device`); серверный счётчик пробных дней `tr:<dev>` больше не сбрасывается переустановкой.
  (3) **Решено не делать** перевод суточного лимита платящих с `cid` на `dev`: переустановка стоит
  подписчику всей локальной истории ради повтора разбора того же дня. (4) Звук таймера пульса, кнопки на
  узких экранах, звук уведомлений на iPhone (`vialchime.wav`) — в `main`, ждут сборки (`696ae7c`).
  (5) **Проверено живьём в тот же день, 13:55 UTC:** реальная покупка в App Store по коду предложения
  (€2,99) → RevenueCat (боевые данные, 1 активная подписка) → приложение сняло пейволл → полный разбор
  без `trial` ($0.0827) → памятка `dayplan-engine` за $0 (утром модель брала $0.0576).
- **2026-09-13** — **Синк устройства честно доходит до разбора; цикл отмечается одной кнопкой; «Помочь улучшить» в EXPERT.**
  (1) **Пульс с прибора.** Живой iPhone, Oura: панель показала пульс 61, в проходе остался «Живой замер
  пульса», разбор написал 60. Лента строится сразу, данные приходят через секунду-две — пересборку
  (`_hkAfterFill`) 4 сентября добавили только для Health; теперь её зовут Oura, Polar, Withings,
  Ultrahuman, WHOOP, Fitbit и тихий рефреш Fitbit. Разбор берёт пульс из поля ленты `#rhr`, а путь Oura
  не звал `applyImportedToSliders` — в разбор уходило прежнее значение. Оба файла (VIA-L и EXPERT).
  (2) **«Сегодня начались месячные»** — карточка «Цикл сегодня» в начале дневного шага 2 (оба
  приложения, 12 языков). День цикла считается от даты в профиле и сам не узнаёт, что цикл пришёл
  раньше; отметку в Oura мы не получаем (OAuth-скоупы без тегов). Кнопка ставит дату = сегодня, есть
  «Отменить», у мужчин и при «Менструаций больше нет» скрыта, гейт её не требует.
  (3) **«Размер текста»** перенесён из EXPERT в VIA-L (в нативке подпись «сохранится на этом
  устройстве» — аккаунта нет; ключ `vialp_bigtext` в резервной копии). Прочие различия «Мой профиль»
  намеренные: «Подписка» и нативное напоминание — только VIA-L, веб-пуш — EXPERT.
  (4) **«Помочь улучшить VIA·L» в EXPERT.** Вместо заглушки — отправка с `tier:'expert'`; воркер хранит
  метку в `data` и считает такие записи в сводках отдельно (`expert:*`, не в общих `*`).
  `research-consent.html?app=expert` — ключи `viae_`, навигация EXPERT, два абзаца на 12 языках
  переписаны (в нативной версии «текст остаётся на устройстве» и «аккаунта нет» — для ведения
  неправда). `viae_pid` проверен: в синхронизацию с карточкой не входит. Абзац в `legal/privacy.html`
  §7-bis ×12, дата 13.09. Включать по умолчанию нельзя — GDPR ст. 9, явное согласие.

- **2026-09-12** — **Ведение стало видно как приложение, а оплата PWA — как строка в кабинете.**
  (1) **Сайт и лендинг перестали врать друг другу.** Блок на сайте Марины звал «Розпочати аналіз»
  и вёл на `interpreter/index.html`, где никакого анализа нет — там ведение за €390/€590.
  Переписан во всех 4 языках в «Ведення в застосунку · VIA-L EXPERT» (8/12 недель, 7 дней
  поддержки, недельный PDF), кнопки ведут на `#how-it-works` и `#plans`; пункт меню
  «Інтерпретатор» → «Ведення в застосунку».
  (2) **На лендинге EXPERT появилось приложение.** Плашка в hero, блок «Ведення проходить у
  застосунку» (оплата → код письмом → установка из браузера → каждый день) и три экрана
  (uk/ru — из PWA, en — кадры нативки без нативной специфики; `interpreter/images-app/`,
  галерея только для uk/ru/en). Значков сторов нет и не будет: EXPERT — PWA вне магазинов.
  (3) **Чистка лендинга:** удалена мёртвая витрина устройств (`DEVICES`/`renderDevices`/
  `selectDevice` — контейнера `#devicesGrid` нет с прошлой переделки) и 34 осиротевших ключа;
  осталось ровно 47 ключей = 47 `data-t` в 12 языках.
  (4) **Язык сайта:** определяется по `?lang=` → сохранённому выбору (общий ключ с ИП) → языку
  браузера → uk; `title`/`description`/og/canonical меняются вместе с языком; `hreflang` uk+ru,
  русская версия на `?lang=ru` и в `sitemap.xml`. Мёртвый ключ `via_lang_lon` убран.
  (5) **`methodology.html`:** дисклеймеры «Анализ носит информационный характер» → «Материалы
  носят…» (24 строки, 12 языков), кнопка возврата больше не зовёт лендинг «Інтерпретатором».
  (6) **Кабинет — оплата клиентом приложения (€30/мес).** Новая таблица `client_payments`
  (`interpreter/cabinet-step10-client-payments.sql`, применена на боевой D1 2026-09-12) —
  отдельно от `payments`, где деньги идут в другую сторону (специалист → платформа) и гасят его
  долг. Ручки `/cabinet/client-payments` и `/cabinet/client-payment`: список, «оплачено до»,
  отметка и удаление; период считается встык к оплаченному, чтобы два перевода не съедали друг
  друга. В карточке клиента — блок с суммой, способом (карта/Hotmart/наличные), итогом и двумя
  предупреждениями: «просрочено» и «доступ выдан дальше оплаченного периода». 16 ключей ×
  12 языков. Проверено живьём: выдача 93 дня + оплата 1 месяца → предупреждение горит.

- **2026-09-04** — **Нативный импорт здоровья доведён до цифр на обеих платформах; канал HRV с Fitbit признан тупиковым.**
  (1) **Кнопка перестала молчать.** «Ввод данных с трекера» запускала чтение в фон на скрытую
  кнопку и сразу скроллила к карточке: при пустом ответе человек видел прочерки без объяснения,
  а «нет данных» и «не выданы разрешения» неразличимы (Apple намеренно не сообщает об отказе в
  чтении). Теперь экран ждёт результата и пишет статус в подпись карточки — «Читаем данные
  из …», при пустом ответе текст про разрешения / ручной ввод / синхронизацию устройства
  (12 языков, `_hkReading`/`_hkEmpty` в обоих тарифах). В Android-мост добавлен
  `healthConnectRequest()` — запрос прав отдельно от обработчика кнопки.
  (2) **Пульс покоя считаем сами.** Ни Google Health в Apple Health, ни мосты Fitbit → Health
  Connect не пишут отдельный тип «пульс покоя» — поле оставалось пустым при живых ночных
  замерах. Фолбэк в обоих мостах: 10-й перцентиль пульса внутри окна сна (перцентиль, не
  минимум — одиночный артефакт не должен занижать сутки). Проверено вживую: **57 bpm**.
  (3) **Живая проверка Android** (Huawei AMN-LX9, Android 9, Health Connect + Health Sync как
  мост от Fitbit): пришли сон 6:54, глубокий 60 мин, SpO₂ 97 %, пульс покоя 57. Прочерками
  остались HRV, температура, VO₂, готовность.
  (4) **Вывод по HRV, чтобы не выяснять заново:** вариабельность из Fitbit наружу не уходит
  вообще — её нет ни в списке типов, которые Google Health пишет в Apple Health, ни в
  категориях Health Sync. Единственный канал — прямой Fitbit Web API, который отключается
  09.2026. После этого HRV с Fitbit у нас не будет ни одним путём; у Oura прямой канал открыт
  и лимит снят, поэтому в рекомендациях трекеров приоритет за ней.
  (5) **Автодетект Oura больше не выдаётся за тренировки** — в `workouts7d` идут только записи
  с `source != 'autodetected'`, автодетект уходит отдельным `activityAuto7d` и отдельной
  строкой промпта с запретом считать его сессиями. Повод: разбор Марины прочитал 20 прогулок
  как 20 тренировок и сделал вывод о перегрузке. Заодно снята вторая половина ошибки — по
  этому числу молча выставлялась частота тренировок (`>=5 → high`) со скрытым вопросом.
  (6) **Экран ежедневного прохода помещается в экран без прокрутки** (скрин с Huawei): ужаты
  верхний отступ, монограмма, шрифты шапки и воздух между карточками источника; юр-ссылки
  отошли от нижней навигации. Прокрутку не запрещаем — при крупном системном шрифте
  содержимое должно прокручиваться, а не обрезаться.
  (7) **Сборка Android чинена:** `versionCode (getenv ?: '1') as Integer` Groovy разбирал как
  вызов с приведением, делегат получал null → «Value is null» на конфигурации. Теперь
  `Integer.parseInt(...)`, правка в `app/scripts/setup-android.js` (папка `app/android/`
  в .gitignore и пересоздаётся). Релизный AAB собран с `VIAL_VERSION_CODE=20260904`.

- **2026-09-03** — **Исследовательский синк переведён в рамку «улучшение», задан срок хранения.**
  Повод: форма Google Play «Приложения для здоровья» требует объявить, ведём ли мы исследование
  на людях. Сверка с текстом правил (`docs/HEALTH-RESEARCH-POLICY-CHECK.md`, с цитатами)
  показала, что **обе** площадки требуют для такого исследования одобрения независимого
  этического комитета — Apple 5.1.3(iv), Google через определение категории «credentialed
  researchers… approved by an IRB or EC». Прежний комментарий в коде утверждал обратное
  (будто у Apple исследование легче «улучшения») — неверно: 5.1.3(i) выводит «improving health
  management» из-под запрета на data mining наравне с research. Что сделано:
  (1) **Текст согласия × 12 языков** — «Помочь улучшить VIA·L» вместо «Участие в исследовании»,
  убрано обещание публиковать результаты и бессрочное хранение; название пункта в профиле
  приложения приведено к тому же. (2) **Срок хранения**: `research_days` — скользящее окно
  730 дней от `received_ts`, чистка порциями раз в сутки из крона; удаление постепенное.
  (3) **Сводки** — новая таблица `research_stats` (период, источник, язык, метрика, n, среднее,
  sd, квартили), считается раз в месяц за закрывшийся месяц, ячейки меньше 5 наблюдений не
  сохраняются; персональных данных нет, поэтому хранятся бессрочно и переживают удаление строк.
  (4) Данные Apple/Health Connect в выборке **не режем** — оснований, как выяснилось, нет;
  в `WEARABLE_RESEARCH_BLOCK` остаётся только Oura (договорный запрет, не сторовый).
  (5) Страница удаления данных и ответы анкеты «Безопасность данных» приведены в соответствие.
  ⏳ Не сделано: применить `research-schema.sql` к D1 и задеплоить воркер.

- **2026-09-02** — **Сборка iOS через Xcode Cloud дошла до устройства + заведён аккаунт Google Play.**
  (1) **iOS:** билд Xcode Cloud из `main` установился на живой iPhone и подтвердил накопленные
  правки юр-ссылок — из профиля открываются «Конфиденциальность» и «Условия», возврат ведёт
  обратно **в профиль** (`?tab=card` в копии `legal/*.html`, которую делает `sync-web.sh`),
  «Наш подход» → «Научная база» (`science.html`) открывается и по ✕ возвращает в «Наш подход».
  Проверено владельцем на устройстве.
  (2) **Google Play:** аккаунт разработчика создан — личный, `viaelcom@gmail.com`,
  идентификация и платёжный профиль на **Кирилла Селиванова** (как ответственное лицо в
  `legal-app/`), контакт для Google — Игорь; монетизация заявлена как покупки в приложении +
  подписки, категории — «ничего из перечисленного» (медицину не отмечаем осознанно).
  Проверка устройства Google (Android 10+ с сервисами) пройдена с чужого телефона: Huawei
  владельца на Android 9, а эмулятор на MacBook Air 2018 — тупик (API 30 несёт Play-сервисы
  2020 года и вечно висит на «Checking info…», API 34 грузится 10 мин и упирается в
  «System UI isn't responding»). Подробности и остаток пути — `docs/ANDROID-PLAY-LAUNCH-PLAN.md`.

- **2026-08-25** — **День живых правок VIA-L/EXPERT + «Помочь улучшить приложение».**
  (1) **Разбор перестал теряться:** ключ воркера смотрел в исчерпанный аккаунт Anthropic
  (`dacigor`), пополнен был другой (`viaelcom`) — отсюда «Your credit balance is too low» вместо
  разбора; ключ заменён. Сырая ошибка API больше не может уйти клиенту как текст разбора и в
  карточку кабинета (`ai_unavailable`). Плюс страховочный кэш готового разбора в KV на 72 ч
  (`/analysis-cache`, ключ `an:<cid>:<день>`) — ответ большой, и при свёрнутом приложении он терялся,
  хотя вызов был оплачен; в приложении появилась кнопка «Получить разбор заново».
  (2) **Триаж давления в трёх слоях:** пороги в KB → готовая оценка тира в промпте → код
  (`_appendTriage` с маркером `[[T]]`, `_fixBpTier`, `_stripUrgencyVerdicts`). Живой случай: при
  165/66 модель написала «слегка выше» и «не срочное состояние»; теперь занижение и вердикты о
  срочности правятся кодом, а блок «На что обратить внимание» открывается любым замером вне
  ориентира (раньше — только симптомами из опросника).
  (3) **Проход:** починены разъезд экрана после импорта Fitbit, перескок блока «Что нового»
  (`_blockComplete` не видела свободный текст), подтверждение перенесённых ответов (чип «Всё так
  же» + `blk-normal`), счётчик блоков не исчезает в конце, «Перейти к анализу» ведёт в ленту.
  Жалоба «С чем вы пришли» вынесена в свой разовый шаг рядом с профилем.
  (4) **Навигация:** «Наш подход», «Мой наставник» и «Мой профиль» стали вкладками с постоянным
  нижним баром и без ✕; приложение понимает `?tab=`.
  (5) **`science.html`** — своя «Научная база» для базового тарифа (12 языков) вместо клинической
  `methodology.html`: там каталог протоколов специалиста с диагнозами и добавками, для App Store
  это неприемлемо. Материал — из `docs/INDEX-SCORING-SOURCES.md`,
  `CYCLE-PHASE-WEARABLES-EVIDENCE.md`, `BLOOD-PRESSURE-EVIDENCE.md`.
  (6) **«Помочь улучшить приложение» (этапы 1–3 пути B):** таблица `research_days` в D1,
  `POST /research/day` с **whitelist на сервере** (свободные тексты не принимаются там, а не в
  клиенте — фильтр не должен зависеть от версии приложения), `POST /research/forget` (GDPR ст. 17),
  `_syncPushDaily` шлёт только при согласии, страница `research-consent.html` с полным текстом и
  переключателем, вход из «Моего профиля». Текст написан без юриста по стандартной структуре
  информированного согласия — решение владельца; EN-эталон + RU/UK, остальные 9 языков после
  ревью. Обе политики (`legal-app/`, `legal/`) описывают и 72-часовой кэш, и opt-in — 12 языков.
  ⚠️ Следствие для подачи: «Data Not Collected» отпало, метки — Health & Fitness / App Functionality /
  Not Linked to You.
  (7) **Согласие переведено на все 12 языков** (вечер того же дня): `research-consent.html` —
  все 9 недостающих языков (es/de/pt/fr/pl/it/he/ja/ko), 31 ключ на язык, порядок и названия языков
  как в приложении, иврит переключает `dir="rtl"`. EN остаётся эталоном: правку текста согласия
  начинать с него и потом раскатывать на 11.

- **2026-08-24 (2)** — **Анализы: красные флаги → семейный врач + «что обычно смотрят».** Постановка
  выбрана после разбора рынка: Oura заказывает панель прямо в приложении только потому, что за
  заказом стоит врач-партнёр (SteadyMD) и лаборатория (Quest), и лишь в США; без врача и во всех
  странах она принимает готовые результаты и объясняет их как educational insight. Мы идём тем же
  путём. Сделано: (1) `_labRedFlags()` — детерминированные пороги из `Infa Cloude/LAB MODULE 1–4`
  (глюкоза, HbA1c, ТГ, ЛПНП, инсулин, ТТГ, пролактин, тестостерон у женщины, ДГЭА-С, гемоглобин,
  рСКФ, АЛТ/АСТ, кальций, CRP, ферритин): называем значение и адресата — СЕМЕЙНЫЙ ВРАЧ, без болезни,
  причины и прогноза; (2) `_labSuggest()` — список «что обычно смотрят» под жалобу клиента с
  таймингом сдачи, в VIA-L строго образовательно (запрещено «сдайте»), в EXPERT как предложение к
  разговору со специалистом; гейт частоты — раз в 7 дней на клиенте (`_labHintDue`, поле `labs_hint`).
  Попутно найдена тихая дыра: в `LAB_TARGETS_EXPERT` пропущенный `+` съедал две последние строки —
  правило «ориентир ≠ доза» не доходило до модели ни разу.

- **2026-08-24** — **Де-медикализация: падежи, орган, анализы, симптомы + петля жалобы.**
  Владелец увидел в проде «когда гормонального баланса становятся менее стабильными» — это была
  фронтовая подмена `_wl` в VIA-L (гормон → «гормонального баланса» в любой позиции). Прогон показал
  тот же класс дефекта в выходном скрабе воркера (`wellnessHardScrub` идёт ПОСЛЕ guardrail, значит
  виден клиенту): «При то, как тело отвечает на сахар», «велнес картина», «Речь о обмен веществ и
  энергия». Сделано: `_wl` доведён до no-op (как в EXPERT с 2026-07-31); в `_FEED_DEMED` ретированы
  подмены ОРГАНА и АНАЛИЗОВ (щитовидная железа, ТТГ, АТ-ТПО, АТ-ТГ, обратный Т3, св.Т3/Т4) и
  симптом-ярлыков (бессонница, ПМС, предменструальный) — канон «прячем диагноз, а не то, о чём
  человек сообщил сам»; прилагательные заменяются через `_adj()` с учётом парадигмы (падеж и регистр
  целы); «абдоминальный жир» — перестановкой, правило поднято выше правил ожирения; `AI_RISK_RE`
  дополнен диагнозами, оставшимися под подменой (гипертиреоз, гипогонадизм, хашимото, тиреоидит,
  остеопороз, остеопения, ожирение, инсомния) — модель перепишет фразу грамотно, подмена стала
  аварийной страховкой. Заодно починен блок «С чем вы пришли»: `_cmpToggle` строил черновик от
  сохранённого профиля и второй клик затирал первый выбор (выбиралось одно вместо двух), а
  пересборка профиля не переносила поле `complaint` — жалоба стиралась и опросник всплывал каждый
  проход (оба тарифа).

- **2026-08-04** — **Распознавание анализов с фото (только EXPERT).** Воркер `POST /labs/parse`:
  base64-картинка → модель извлекает ТОЛЬКО числа по белому списку из 44 ключей и возвращает JSON;
  чужие ключи и нечисловые значения отбрасываются кодом, файл нигде не сохраняется (живёт в памяти
  запроса). В VIA-L этого нет намеренно: там данные не покидают устройство, и фото бланка сломало бы
  и обещание, и privacy-манифест App Store. Модель — **Sonnet, не Haiku**: на тесте Haiku уверенно
  путала ЛПВП с ЛПНП даже с явной подсказкой; после перехода 10 из 10 значений и дата распознаны верно.
  В промпт вложена таблица соответствий русских/украинских названий ключам.
  Клиент (EXPERT): кнопка «Распознать с фото» в секции «Анализы», сжатие на устройстве до 1600 px,
  затем **обязательный экран подтверждения** — значения в единицах бланка, всё правится, сохраняется
  только подтверждённое и через тот же `_labSave`, что и ручной ввод. Автосохранения нет by design.

- **2026-08-04** — **Дозы в EXPERT: граница по АВТОРУ, а не по тарифу (решение владельца).** Спор был
  не о том, можно ли дозировать добавки — специалист вправе, это его работа, и в кабинете есть протокол
  с колонкой «Доза». Вопрос был, кто называет цифру. Решено: из автоматического текста ИИ дозы уходят
  («название есть, специалист скажет сколько, на банке инструкция»), протокол специалиста не трогаем.
  В `handleAnalyze` для не-велнес ветки включены `_stripSupplementDoses` + `_stripHerbLines`;
  предупреждения о травах (`_HERB_WARN_RE`) сохраняются, терминология не трогается.
  ⚠️ При проверке всплыли два молчаливых бага: `_SUPP_RE` не знал сокращений (D3, K2, Q10, EPA/DHA,
  креатин, NAC) — строки с ними проходили мимо фильтра; и в `_DOSE_RE` стояло `МЕ\b`, а `\b` в JS
  не видит кириллицу, поэтому «2000–3000 МЕ» не ловилось НИКОГДА. Оба исправлены (границы через
  `(?![\p{L}])`, флаг `u`). Живой прогон после правки: доз нет, порции еды и белок в г/кг целы.

- **2026-08-04** — **Сверки по расширенной панели + ориентиры для EXPERT.** В `_labCrossChecks`
  добавлены детерминированные правила: кальций+ПТГ вместе, кальций при высоком витамине D, антитела
  щитовидной (проверяются однократно), биотин перед тиреоидным анализом, тестостерон общий без
  свободного при известном ГСПГ, B12 на метформине/ИПП, гомоцистеин с невысокими B12/фолатом,
  HOMA-IR ≥2.5, ПСА >4 у мужчины. Проверено прогоном по 10 сценариям: срабатывают точно, у мужчины
  с ферритином и у женщины с ПСА молчат. В промпт EXPERT добавлен блок `LAB_TARGETS_EXPERT` —
  функциональные ориентиры (обмен, липиды, железо, витамины, щитовидка по возрасту, печень/почки,
  кости, половые гормоны с оговоркой о скачках в переходе); в VIA-L его нет намеренно: «вы вне
  оптимума» = вердикт о здоровье.

- **2026-08-04** — **Панель анализов: 11 маркеров → 45, группами.** По аудиту
  `docs/LABS-PANEL-AUDIT.md`: добавлены щитовидная панель (fT4/fT3/АТ-ТПО/АТ-ТГ), половые гормоны
  (E2/ФСГ/ЛГ/прогестерон/тестостерон общий и свободный/ГСПГ/ДГЭА-С/пролактин/ПСА), железо и кровь,
  липиды до полного профиля (общий/HDL/Лп(а)), инсулин, печень и почки, кости, витамины и
  гомоцистеин — с единицами и коэффициентами пересчёта. В UI `LAB_GROUPS`: 8 свёрнутых групп
  (базовое открыто, у групп с данными — счётчик), иначе 45 полей стеной. **HOMA-IR не поле, а
  расчёт** (`_homaLine`, глюкоза×инсулин/22.5). В воркере расширены словари N/U — без них новые
  ключи молча выпадали бы из разбора.
  Пороги из исходного PDF взяты НЕ как есть: в промпт вписаны правила «ТТГ растёт с возрастом
  (не применять 2.5 к 40+)», «рСКФ снижается с возрастом», «у ПСА нет оптимума, только >4 → уролог»,
  «HDL не «чем выше тем лучше»», «половые гормоны в переходе скачут — только контекст», «Лп(а) и
  антитела однократны». Плюс запрет ярлыков на цифру («сниженный эстрадиол» у мужчины с E2 22 —
  живой прогон это поймал) и скраб вердиктов «в верхней норме»/«на нижней границе нормы».
...
- **2026-08-03** — **Черновики договоров (E1+E2).** `docs/OFFER-CLIENT-SPECIALIST-DRAFT.md` —
  ...
  выдавать EXPERT, деньги (€30/мес абонплата `PLATFORM_FEE_EUR` + €30/мес софт за клиента из `grants`),
