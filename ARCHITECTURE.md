...
| Продукт | Что это | Точка входа | Оплата |
|---|---|---|---|
| **Программы ведения** | Сопровождение нутрициолога (Менопауза / Андропауза / Антивозрастное / Эстроген) | Лендинг + 4 program-страницы | Hotmart → анкета → сессии |
| **Интерпретатор (ИП)** | Веб-приложение для расшифровки данных носимых устройств (Oura и др.) | `interpreter/` | Hotmart → код доступа |
...
| Файл | Назначение | Шагов | AI | i18n |
|---|---|---|---|---|
| `index.html` | Лендинг ИП: галерея устройств (3D) + тарифы | — | — | 12 яз. |
| `methodology.html` | «Научная база» (`noindex`) | — | — | 12 яз. |
| `interpreter-vio.html` | VIO — бесплатный | 6 | — | 12 яз. |
| `interpreter-pro.html` | PRO | 14 | ✓ | 12 яз. |
| `interpreter-pro-expert.html` | PRO + EXPERT | 16 | ✓ + Expert | 12 яз. |
| `interpreter-elite.html` | ELITE (8w / 12w) | 16 | ✓ + Expert + onboarding | 12 яз. |
...
2. Клиент открывает /interpreter → выбирает тариф → открывает interpreter-*.html
        ...
        │
3. Пошаговый ввод биометрии (6 / 14 / 16 шагов) — вручную или импортом устройства
