...
под профиль. Спека платформы: [`docs/PLATFORM-MODEL.md`](docs/PLATFORM-MODEL.md). Механика
платформы — §11.8 (офлайн-модель); онлайн-модель будет описана отдельно.
...
(специалист ведёт пациентов по миру удалённо, как основатель в EXPERT/ELITE) — обсуждается отдельно.
