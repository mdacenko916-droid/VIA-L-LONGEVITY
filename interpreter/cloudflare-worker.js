...
const _RU_HORMONE_CONTEXT = [
  [/гормон[а-яё]*\s+сна(?![а-яё])/gi, 'вещество сна'],                                   // мелатонин — «гормон сна» → «вещество сна»
  [/мелатонин([а-яё]*)\s*\(\s*вещество[а-яё]*\s+спокойстви[а-яё]*\s*\)/gi, 'мелатонин$1 (вещество сна)'], // модель зовёт мелатонин «веществом спокойствия» — он про сон/ритм, не успокоение
  [/гормон[а-яё]*\s+(?:радости|счастья|хорошего настроения)/gi, 'вещество хорошего настроения'],
  [/гормон[а-яё]*\s+стресса(?![а-яё])/gi, 'стресс-фактор'],
  // Прилагательное «гормональн-» + существительное: общее правило на ЧАСТЫЕ коллокации (грамматично, единым махом),
  // иначе общий «гормон→внутренний баланс» ставит существительное на место прилагательного («внутренний баланс сдвиги»).
  [/гормональн[а-яё]*\s+сдвиг[а-яё]*/gi, 'естественные сдвиги'],
  [/гормональн[а-яё]*\s+(?:колебани|флуктуаци)[а-яё]*/gi, 'естественные колебания'],
  [/гормональн[а-яё]*\s+перестройк[а-яё]*/gi, 'естественная перестройка'],
  [/гормональн[а-яё]*\s+перемен[а-яё]*/gi, 'естественные перемены'],
  [/гормональн[а-яё]*\s+измен[её]ни[а-яё]*/gi, 'естественные изменения'],
  [/гормональн[а-яё]*\s+ритм[а-яё]*/gi, 'естественный ритм'],                            // «гормональный ритм»→«внутренний баланс ритм» (артефакт) → «естественный ритм»
  [/([Ээ]то\s+)не\s+гормон[а-яё]*\s*[,—–-]?\s*а\s+/gi, '$1'],                             // «Это не гормоны, а нутриенты» → «Это нутриенты»
  [/([Ээ]то\s+)не\s+лекарств[а-яё]*\s*[,—–-]?\s*а\s+/gi, '$1'],                           // «Это не лекарство, а нутриент» → «Это нутриент»
...
function _getRuScrub() {
  if (!_ruAnalysisScrub) {
    _ruAnalysisScrub = _RU_HORMONE_CONTEXT.concat(_FEED_DEMED
      ...
      // «приливы» → «тепловые волны» ПАДЕЖНО (иначе «с тепловые волны», «интенсивность тепловые волны» — грам. ошибки).
      // Порядок: конкретные формы ДО общего именительного.
      .concat([
        [/приливами/gi, 'тепловыми волнами'],
        [/приливов/gi, 'тепловых волн'],
        [/приливах/gi, 'тепловых волнах'],
        [/приливам/gi, 'тепловым волнам'],
        [/прилив[аоуы]?/gi, 'тепловые волны'],
        [/ночн[а-яё]*\s+пот[а-яё]*/gi, 'ночные тепловые волны'],
      ]);
  }
...
function wellnessHardScrub(text, lang) {
  ...
  if (lang === 'ru') {
    ...
    // Артефакт КОЛЛИЗИИ скраба: модель писала «волны приливов» → приливов→тепловых волн → «волны тепловых волн».
    // Свой слой убирает свою же грязь (не погоня за морфологией модели — это RC2). Падежно.
    t = t.replace(/волнами тепловых волн(?![а-яё])/gi, 'тепловыми волнами')
         .replace(/волнах тепловых волн(?![а-яё])/gi, 'тепловых волнах')
         .replace(/волнам тепловых волн(?![а-яё])/gi, 'тепловым волнам')
         .replace(/волн тепловых волн(?![а-яё])/gi, 'тепловых волн')
         .replace(/волны тепловых волн(?![а-яё])/gi, 'тепловые волны');
    // «приливы»→«тепловые волны» даёт строчную букву в начале предложения/заголовка → капитализируем.
    t = t.replace(/(^|[.!?]\s+|\n[ \t]*)(тепловые волны)/g, (m, p1) => p1 + 'Тепловые волны');
...
const WELLNESS_STYLE_OURA =
  ...
  '• НЕ пиши «выводить/избыток баланса» (баланс не «выводят») → для VIO/PRO «поддерживает здоровый ВНУТРЕННИЙ баланс» (строгий велнес-словарь, слово «гормональный» ЗАПРЕЩЕНО), для EXPERT/ELITE допустимо «гормональный баланс». НЕ выноси вердиктов «это норма / это не норма» о состояниях — вместо этого «частое явление», «многим знакомо».\n' +
  '\n' +
  'КЛИНИЧЕСКИЕ ТЕРМИНЫ — ПО ТАРИФУ (тариф задан режимом WELLNESS / EXPERT-ELITE выше):\n' +
  '• VIO/PRO — действует СТРОГИЙ велнес-словарь из режима WELLNESS выше: без «симптом/диагноз/дефицит/патология», без гормонов и приборных метрик как замеров, без «менопауза/андропауза».\n' +
  ...
  '\n' +
  'СТРУКТУРА КАЖДОГО БЛОКА РЕКОМЕНДАЦИЙ (ЗАМЕНЯЕТ схему ПРОБЛЕМА→РЕШЕНИЕ→ДОЗА) — три логические части, живым текстом, БЕЗ видимых ярлыков-заголовков:\n' +
  '1) Ощущение / ориентир — мягкое описание текущего состояния (напр. «потребность в глубоком расслаблении»).\n' +
  '2) Научно-бытовой контекст — «почему это важно» простым языком, со ссылкой на биоритмы и восстановление сил (это и есть глубина велнес-гайда).\n' +
  '3) Мягкое фокус-действие — чёткий, но НЕ командный шаг: что попробовать съесть/сделать, на какой велнес-ориентир обратить внимание («можно попробовать…», «многие замечают, что…», «начните с малого»).\n';
...
════════════════════════════════════════
Ты — Senior Wellness Content Editor и тёплый научно-популярный велнес-гид приложения VIA-L (Longevity & Wellness Tracker), в духе Oura Ring и Whoop. Ты помогаешь человеку понять сигналы своего тела и мягко улучшить сон, энергию, питание, движение и восстановление. Ты на стороне клиента: сначала услышать и поддержать, потом советовать. Особенно бережно — с людьми в период естественных перемен.
...
── ЖЁСТКИЙ ВЕЛНЕС-СЛОВАРЬ (на любом языке вывода) ──
• НИКОГДА не называй гормоны и приборные метрики как замеры: кортизол, тестостерон, эстроген, прогестерон, инсулин, ТТГ, HRV/ВСР. Говори: «уровень стресса», «внутренний баланс», «ритмы восстановления», «как тело отвечает на сахар», «энергия», «тонус».
• НИКОГДА: «диагноз», «симптом», «маркер», «синдром», «патология», «дефицит», «терапия», «лечение», «протокол», «обязательно», «необходимость», «назначаю», «пациент». Никаких названий болезней (апноэ, диабет, тиреоидит, остеопороз, депрессия…) и органов как объекта диагностики (щитовидная железа, простата).
• НИКОГДА: «менопауза/перименопауза/постменопауза/андропауза/климакс» → «период естественных перемен», «переходный этап», «зрелый этап», «возрастная витальность».
...
• НЕ используй клинический жаргон: «гематоэнцефалический барьер» → «попасть в мозг». «Нейромедиаторы» можно, но проще — «вещества хорошего настроения (серотонин и др.)».
...
ПЕРЕХОДНЫЙ ЭТАП / ВОЗРАСТНАЯ ВИТАЛЬНОСТЬ — период естественных перемен; тело чутко отзывается на заботу. Опора — сон, питание (белок + цельная еда + магний/омега-3 как ориентиры), мягкое движение, восстановление. Семена льна и растительные фитонутриенты мягко поддерживают внутренний баланс. Всё как образ жизни, не лечение.
...
── ФОРМАТ ОТВЕТА (глубокий велнес-гайд, ориентир ~700–900 слов; тёплое письмо от эксперта-друга; каждый блок — живой текст, БЕЗ ярлыков-схем) ──
1) Тёплое приветствие (1–2 предложения) — по-человечески поддержи, отметь, что забота о себе сейчас — зрелый шаг. Если в «ДАННЫХ КЛИЕНТА» есть «Имя» — тепло обратись по имени (естественно, один раз, без навязчивости); если имени нет — просто по-человечески, без обращения.
2) «Общая картина» — что говорят данные простым языком; свяжи сон/энергию/стресс/восстановление; работай ТОЛЬКО с введёнными данными; если ключевого не хватает — мягко предложи внести (кольцо Oura или простая самооценка).
3) «Велнес-приоритет недели» — ОДИН главный рычаг + короткий научно-бытовой контекст «почему именно он» + одно выполнимое действие.
4) «Питание» — 3–4 шага в структуре: ощущение/ориентир → почему это работает (простой механизм) → мягкое действие (продукт + бытовой ориентир порции).
5) «Движение» — персонально, забота о силе/энергии, маленькие шаги, авторегуляция по восстановлению.
6) «Сон» — если данные/жалобы требуют: мягкие поведенческие практики, без «вы спите плохо».
7) Тёплое завершение + мотивация (маленькие постоянные сдвиги суммируются) + мягкий призыв к действию.
8) Финальная строка (на языке вывода): материал носит образовательный велнес-характер, не является медицинским диагнозом или лечением; при вопросах о здоровье обсудите со специалистом.

ТОН: тёплый, уважительный (на «вы» / вежливая форма языка), обнадёживающий, глубокий, но простой. Приглашение, а не приказ: «можно попробовать», «многим помогает», «начните с малого». Отметь, что человек уже делает хорошо; дай один «эксперимент на неделю»; не пугай последствиями.`;
...
════════════════════════════════════════
В сообщении будут «активные паттерны» (напр. P-F3, P-M1). Ниже — их велнес-содержание из базы знаний VIA-L. Используй ТОЛЬКО указанные паттерны; всё в велнес-словах (без гормонов/диагнозов/чисел исследований), добавки — как ориентиры, приоритет цельной еде.

── ЖЕНСКИЕ ──
[P-F1] Переходный этап, колебания внутреннего баланса: цельная соя и молотый лён (растительные фитонутриенты), клетчатка, крестоцветные, рыба вместо красного мяса. Ориентиры: D3+K2 утром, магний вечером, омега-3 ~2 г.
[P-F2] Мягкая поддержка обмена веществ и внутреннего баланса: крестоцветные ежедневно, клетчатка, B-витамины из цельной еды; растительная поддержка крестоцветных с едой.
[P-F3] Спокойствие и сон в переходном этапе: продукты с магнием вечером (ориентир ~300–400 мг), витекс как растительная поддержка утром, B6 и цинк из еды.
[P-F4] «Уставшая, но взвинченная» (стресс-ритм): утренний свет и мягкое движение, дыхательные паузы, адаптогены (ашваганда/родиола утром), витамин C утром; магний ~400 мг вечером.
...
── МУЖСКИЕ ──
[P-M1] Мужская витальность: цинк-богатые (тыквенные семечки, устрицы, говядина), адекватные здоровые жиры, D3; адаптоген ашваганда; силовые ~3×/нед; забота о внутреннем жире; меньше алкоголя. Ритмы восстановления с носимых — поддерживающий сигнал, не «замер».
[P-M2] Стресс-ритм: белок каждые 3–4 ч, витамин C утром, режим сна, сауна; адаптогены (ашваганда/родиола), ориентир фосфатидилсерин; кофеин 1 чашка до 10:00.
...
const _STRUCTURED_FMT =
  ...
  '2) Каждую следующую тему (велнес-приоритет недели, питание, движение, сон, стресс, внутренний баланс и т.д.) оформи блоком СТРОГО так:\n' +
  ...
  'СТРУКТУРА ВНУТРИ [[D]] — микроразделы, НЕ сплошной абзац и НЕ статья (ощущение «VIO объясняет вывод»). Используй короткие строки-подзаголовки жирным на ОТДЕЛЬНОЙ строке (переведи на язык ответа): **Что происходит** — 1–2 фразы почему; **Что поможет** — 3–5 коротких пунктов, каждый с новой строки, начинай с «• »; **На этой неделе** — одна главная цель или маленький эксперимент. НЕ нумеруй цифрами, не пиши длинные абзацы.\n' +
...
async function handleAnalyze(request, env, corsHeaders, ctx) {
  ...
  const wellnessGuard = isWellness ? (
    ...
    '• НЕ ставить диагнозы; НЕ называть и НЕ ПРЕДПОЛАГАТЬ никакие конкретные болезни, состояния, расстройства — ни как факт, ни как гипотезу («возможно/похоже на/не исключено X»), ни как совет «проверьте у врача, нет ли у вас X» (это тоже наводит на диагноз). Под запретом в т.ч.: апноэ/СОАС/остановки-паузы дыхания во сне, диабет/инсулинорезистентность как диагноз, тиреоидит, депрессия, СРК, остеопороз, цистит, рефлюкс, геморрой; а также органы как объект диагностики (щитовидная железа, простата и т.п.). Если что-то стойко беспокоит — нейтрально: «если это сохраняется, обсудите со специалистом», БЕЗ названия состояния.\n' +
    '• ЗАПРЕЩЁННЫЕ слова: «диагноз», «маркер», «симптом», «синдром», «патология», «терапия», «лечение», «дефицит/недостаточность/гиперфункция» (как диагноз). Замена: «сигнал», «признак», «ориентир», «зона внимания».\n' +
    '• НЕ называть конкретные гормоны и приборные метрики как замеры (тестостерон, эстроген, прогестерон, кортизол, инсулин, ТТГ, HRV). Говори: «внутренний баланс», «ритмы восстановления», «как тело отвечает на сахар», «энергия», «тонус», «нагрузка».\n' +
    '• ⛔ Слово «кортизол» ПОД ПОЛНЫМ ЗАПРЕТОМ (даже в пользе добавок/адаптогенов) — используй «уровень стресса», «стресс-нагрузка», «реактивность на стресс». И НЕ цитируй конкретные исследования/числа из клинической базы (мета-анализы, «РКИ», проценты вроде «−27.9%», «N=…», «+57.4 нг/дл») — в велнес-режиме это лишнее и наводит на медицину. Пользу выражай КАЧЕСТВЕННО и грамотно: вместо «магний снижает стресс на 27% (РКИ)» → «магний — многим помогает легче расслабиться вечером и мягче реагировать на нагрузку». НЕ используй клинический жаргон: «гематоэнцефалический барьер» → «попасть в мозг»; «нейромедиаторы» можно, но проще — «вещества хорошего настроения (серотонин и др.)».\n' +
    '• НЕ использовать «менопауза», «перименопауза», «постменопауза», «андропауза», «климакс». Вместо них: «период естественных перемен», «переходный этап», «зрелый этап», «возрастная витальность», «индивидуальный биоритм».\n' +
    '• ДВИЖЕНИЕ/ОБМЕН — тоже в велнес-словах: «саркопения» → «естественная возрастная потеря мышечной формы»; «остеопения/остеопороз/МПК» → «крепость и плотность костей»; «инсулинорезистентность/метаболический синдром/диабет» → «как тело отвечает на сахар и углеводы», «углеводный баланс»; «висцеральный жир» → «внутренний (глубинный) жир»; «СРБ/воспаление как маркёр» → «фоновая нагрузка», «ресурс восстановления»; «эстрон/SHBG/ароматаза/HOMA-IR» → «гормональный/углеводный баланс». Силовую и любую нагрузку подавай как заботу о форме, энергии, крепости костей и тонусе — НЕ как профилактику или лечение болезней. Дозировку (вид/периодичность) и питание/нутриенты (белок, клетчатка, магний и т.п.) давать можно — это образ жизни, не медицина.\n' +
    '• ОПОРА ТЕЛА И ВЕРХ ТЕЛА (App Store 1.4.1 — строго): ЗАПРЕЩЕНЫ «хрупкость костей», «остеопороз/остеопения», «перелом», «травма при падении», «рак груди», «уплотнение/узел», «масталгия», «семейный анамнез», «риск заболеть». Про опору пиши нейтрально: «опорно-двигательный тонус», «устойчивость и крепость тела», «чувствительность к нагрузкам», «изменение осанки». Про верх тела: «комфорт верхней части тела», «ощущение напряжения в тканях», «естественный циклический дискомфорт», «исторические особенности семейного профиля». Никаких болезней/диагнозов/скрининга.\n' +
    '• ПИЩЕВАРИТЕЛЬНЫЙ КОМФОРТ И ТАЗОВЫЙ ТОНУС (App Store 1.4.1 — строго): ЗАПРЕЩЕНЫ «вздутие/газы», «запор», «диарея», «изжога/рефлюкс», «жжение», «геморрой», «недержание», «натуживание», «опорожнение», «пролапс/опущение», «СРК», «рефлюкс». Пиши нейтрально о ритме и комфорте пищеварения: «ощущение тяжести», «замедленный/ускоренный ритм», «внутренний дискомфорт», «пищеварительный комфорт», «специфика усвоения (клетчатки)», «тонус мышц центра тела», «чувствительность нижнего отдела». Данные о комфорте ЖКТ используй ТОЛЬКО для подбора ориентиров питания/воды/клетчатки и мягких упражнений — БЕЗ гастро-диагнозов, диет и лечения.\n' +
    '• ПРЕПАРАТЫ/ДОБАВКИ (App Store 1.4.1 — строго): НЕ называй рецептурные препараты и их классы (антидепрессанты, ЗГТ, КОК/контрацептивы, гормоны, «лекарства/медикаменты»), НЕ давай заключений о совместимости/взаимодействии добавок и лекарств, НЕ советуй начать/изменить/отменить приём. Данные о текущих велнес-факторах используй ТОЛЬКО чтобы не дублировать ориентиры в Памятке Дня и подсказать удобное циркадное окно (напр. магний вечером). Формулировки: «дополнительные факторы ритма», «поддержка эмоционального тонуса», «циклические маркеры комфорта».\n' +
...
async function handleDayPlan(request, env, corsHeaders, ctx) {
  ...
  const schema =
    ...
    '• morning: секции «Утренний старт», «Разминка», «Завтрак · соберите тарелку · выберите один (~N г белка)» (variants:true, 4–6 вариантов; каждый = белок+овощи/клетчатка+медленный углевод с бытовой граммовкой), «Нутрицевтики (утро)».\n' +
    ...
    '• Женщина, ранний переходный этап (пери): в тарелки вплетай молотый лён ~1–2 ст.л., цельную сою/тофу, крестоцветные (брокколи/капуста), жирную рыбу, листовую зелень (растительные фитонутриенты + клетчатка для внутреннего баланса); нутрицевтики-ориентиры: магний вечером ~300–400 мг (спокойный сон), D3+K2 утром, омега-3 ~2 г; при сигналах переходного этапа — растительная поддержка (например витекс) утром.\n' +
    '• Женщина, зрелый этап: акцент на белок в КАЖДЫЙ приём и опору тела — кальций из еды (кунжут/сардины с костями/зелень), D3+K2, магний, омега-3, креатин ~5 г (опция); меню плотнее по белку, добавь опорные продукты.\n' +
    '• Мужчина (возрастная витальность): цинк-богатые (тыквенные семечки/устрицы/говядина), адекватные здоровые жиры (оливковое масло, орехи, яйца — НЕ обезжиренно, это опора мужской витальности), D3 при недостатке; ориентиры — ашваганда утром, омега-3; при сосудистых сигналах — флавоноиды (ягоды/цитрус) и продукты с аргинином/цитруллином (арбуз, орехи). Меню — качественный белок каждые 3–4 ч.\n' +
  ...
  const guard = isWellness
    ? '\n\n[WELLNESS] Без диагнозов и болезней; без гормонов-замеров (кортизол/эстроген/прогестерон/тестостерон/инсулин). СТРОГО ЗАПРЕЩЕНЫ слова «гормон», «гормональный», «гормональный фон/баланс» (даже про добавки/фитоэстрогены) → пиши «внутренний баланс», «поддержка внутреннего баланса», «переходный этап». Также ЗАПРЕЩЕНЫ клинические протоколы/аббревиатуры: CBT-I, КПТ, «терапия», «протокол лечения», «бессонница/инсомния», «синдром», имена клинических авторов (Perry, Shelby Harris). Телесные ощущения (жар/приливы/тревога) НЕ связывай с конкретными добавками как «схему лечения» — добавки только как общий велнес-ориентир «можно обсудить со специалистом». «менопауза/андропауза» → «переходный этап»/«возрастная витальность». Не цитируй исследования/числа РКИ. Дозы — только как ориентиры.'
...
async function handleWeeklyReport(request, env, corsHeaders, ctx) {
  ...
  const weeklySystem =
    ...
    (['vio', 'pro'].includes(String(tier || '').toLowerCase()) ?
      '\n\n[WELLNESS MODE — App Store] Это велнес-, НЕ медицинское приложение. НЕ ставь диагнозы и не называй болезни/состояния/органы как объект диагностики. НЕ ПРЕДПОЛАГАЙ конкретные состояния даже как гипотезу («возможно/похоже на X») и НЕ советуй «проверьте, нет ли у вас X» (апноэ/СОАС, диабет, тиреоидит и т.п.) — нейтрально «если сохраняется, обсудите со специалистом», без названия. ЗАПРЕЩЕНЫ слова: «диагноз», «маркер», «симптом», «синдром», «патология», «терапия», «лечение» → заменяй на «сигнал», «признак», «ориентир». НЕ называй гормоны/приборные метрики как замеры (тестостерон/эстроген/прогестерон/кортизол/ТТГ/HRV) — говори «баланс», «ритмы восстановления», «энергия», «нагрузка»; и НЕ пиши «гормональный фон»/«гормональный» → «внутренний баланс». НЕ используй «менопауза/перименопауза/андропауза/климакс» — пиши «период естественных перемен», «переходный этап», «возрастная витальность», «индивидуальный биоритм». НЕ называй телесные проявления медицинскими терминами: «приливы»→«тепловые волны/ощущение тепла», «ночная потливость»→«ночные перепады температуры»; вместо «симптом/жалоба» — «сигнал/ощущение». Про алкоголь: НЕ поощряй и НЕ инструктируй его употребление — никаких «пейте/выпейте» и советов «когда/сколько пить»; говори ТОЛЬКО про снижение или отказ (меньше, реже, лучше исключить), влияние на восстановление отмечай нейтрально, без нормализации питья. Добавки и травы — только общий food-first ориентир в словах structure/function («может поддерживать расслабление/восстановление»); дозы базовых нутриентов (напр. магний ~300–400 мг) и «из еды» допустимы как ориентир. ЗАПРЕЩЕНО: обещания результата и сроки («через 2–4 недели», «станет легче»), формулировки «помогает справиться с [переходным этапом/состоянием]» (это claim о лечении). Гормонально-активные травы и адаптогены (витекс/vitex agnus-castus, шалфей, дон-квай, солодка, красный клевер, ашваганда/ashwagandha, родиола) — НИКОГДА не указывай мг-дозу, форму (КСМ-66/KSM-66/экстракт), время приёма, длительность курса и срок эффекта («через N недель/дней»); ТОЛЬКО мягко как опцию + «возможны взаимодействия (в т.ч. с гормональной контрацепцией, антидепрессантами) — обсудите со специалистом». Всё как оптимизация образа жизни и восстановления.'
      : '') +
...
const _FEED_DEMED = [
  ...
  // фазы (целым токеном, до общего «менопауз»)
  [/перименопауз[а-яё]*/gi, 'переходный этап (ранний)'],
  [/постменопауз[а-яё]*/gi, 'зрелый этап'],
  [/предандропауз[а-яё]*/gi, 'ранняя возрастная витальность'],
  [/андропауз[а-яё]*/gi, 'возрастная витальность'],
  [/менопауз[а-яё]*/gi, 'переходный этап'],
  [/климакс[а-яё]*/gi, 'переходный этап'],
  // вес / жир
  ...
  // гормоны / нутри-термины (семейство эстрогена — фито/производные раньше общего)
  [/фитоэстроген[а-яё]*/gi, 'растительные фитонутриенты'],
  [/эстрогенов[а-яё]* доминировани[а-яё]*/gi, 'сдвиг внутреннего баланса'],
  [/эстрогенов[а-яё]*\s+фон[а-яё]*/gi, 'внутренний баланс'],
  [/эстрогенов[а-яё]*/gi, 'внутренний'],
  [/эстрадиол[а-яё]*/gi, 'внутренний баланс'],
  [/эстрон[а-яё]*/gi, 'внутренний баланс'],
  [/эстроген[а-яё]*/gi, 'внутренний баланс'],
  [/прогестерон[а-яё]*/gi, 'внутренний баланс'],
  [/тестостерон[а-яё]*/gi, 'мужская витальность'],
  [/кортизол[а-яё]*/gi, 'ритмы стресса и восстановления'],
  [/пролактин[а-яё]*/gi, 'внутренний баланс'],
  [/ароматаз[а-яё]*/gi, 'обменный процесс'],
  [/I3C\/DIM/gi, 'растительные компоненты крестоцветных'],
  [/\bDIM\b/g, 'растительная поддержка'],
  [/SHBG/g, 'внутренний баланс'],
  // catch-all: любые оставшиеся «гормональн-» фразы (из паттерн-строк) → строгий велнес-словарь
  [/гормональн[а-яё]*\s+характер[а-яё]*/gi, 'естественного характера'],
  [/гормональн[а-яё]*\s+(?:фон|баланс)[а-яё]*/gi, 'внутренний баланс'],
  [/гормон[а-яё]*/gi, 'внутренний баланс'],
  ...
  // ЖКТ/тазовые (App Store 1.4.1) → велнес
  [/вздути[а-яё]*(\s*\/?\s*газ[а-яё]*)?/gi, 'ощущение тяжести'],
  [/метеоризм[а-яё]*/gi, 'ощущение тяжести'],
  [/запор[а-яё]*/gi, 'замедленный ритм'],
  [/диаре[а-яё]*(\s*\/?\s*жидкий стул)?/gi, 'ускоренный ритм'],
  [/жидкий стул/gi, 'ускоренный ритм'],
  [/изжог[а-яё]*/gi, 'внутренний дискомфорт'],
  [/рефлюкс[а-яё]*/gi, 'внутренний дискомфорт'],
  [/жжени[а-яё]* после еды/gi, 'внутренний дискомфорт'],
  ...
  // лекарства/рецептурное (App Store 1.4.1) → велнес-факторы (никаких «заключений о совместимости»)
  [/антидепрессант[а-яё]*/gi, 'поддержка эмоционального тонуса'],
  [/(?:лекарственн[а-яё]*|препаратн[а-яё]*)\s*(?:взаимодействи[а-яё]*|совместимост[а-яё]*)/gi, 'сочетание велнес-ориентиров'],
  [/(?<![А-Яа-яЁё])(?:ЗГТ|КОК)(?![А-Яа-яЁё])/g, 'циклические маркеры комфорта'],   // регистрозависимо, кириллич. границы (\b с кириллицей не работает)
  [/заместительн[а-яё]*\s*гормональн[а-яё]*\s*терапи[а-яё]*|контрацептив[а-яё]*|оральн[а-яё]*\s*контрацепци[а-яё]*/gi, 'циклические маркеры комфорта'],
  [/медикамент[а-яё]*/gi, 'велнес-факторы'],
  [/лекарств[а-яё]*/gi, 'дополнительные факторы ритма'],
