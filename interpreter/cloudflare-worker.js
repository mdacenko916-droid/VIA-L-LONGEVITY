...
async function handleDayPlan(request, env, corsHeaders, ctx) {
  ...
  const schema =
    ...
    'КРАТКОСТЬ ОБЯЗАТЕЛЬНА (это памятка, не статья): каждый пункт — ОДНА короткая фраза, ≤ ~12 слов, без длинных пояснений и без технических деталей (не «лампа 5000 К»). Секций на раздел — до 4; пунктов в секции — 2–4 (в меню 4–6 коротких вариантов). НЕ делай отдельный акцент на свете. Уложись целиком в ответ. Верни ТОЛЬКО JSON.';
