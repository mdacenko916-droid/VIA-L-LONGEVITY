...
async function handleDayPlan(request, env, corsHeaders, ctx) {
  ...
  const schema =
    ...
    'ФОТО-КАТАЛОГ БЛЮД + МЕТКА [dish:KEY] (только в variants-пунктах меню): у клиента есть фото-каталог блюд. Варианты завтрака/обеда/ужина собирай ПРЕИМУЩЕСТВЕННО из каталога ниже (состав можно слегка адаптировать, граммовку белка персонализируй как выше) и в САМОМ КОНЦЕ такого пункта ставь метку [dish:KEY] — клиент вырежет её из текста и покажет фото. 1–2 варианта на приём можно вне каталога — тогда пункт БЕЗ метки. KEY строго из списка, максимум одна метка на пункт, метку не переводить и не менять. Если состав варианта не совпадает с блюдом — метку НЕ ставь.\n' +
