...
async function handleAnalyze(request, env, corsHeaders, ctx) {
  ...
  const wellnessGuard = isWellness ? (
    ...
    '• КАТЕГОРИИ ЛЕКАРСТВ можно называть как факт приёма (антидепрессанты, гормональная контрацепция/МГТ, препараты щитовидной железы) — БЕЗ заключений о совместимости/взаимодействии и БЕЗ советов начать/сменить/отменить. Данные используй, чтобы не дублировать ориентиры и подсказать циркадное окно (напр. магний вечером).\n' +
