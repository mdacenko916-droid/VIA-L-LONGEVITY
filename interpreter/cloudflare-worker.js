...
async function handleAnalyze(request, env, corsHeaders, ctx) {
  ...
  const wellnessGuard = isWellness ? (
    ...
    '• НЕ ставить диагнозы и НЕ называть болезни/состояния как диагноз.\n' +
    '• НЕ использовать слова «менопауза», «перименопауза», «постменопауза», «андропауза», «климакс» как диагноз. Вместо них: «период гормональных перемен», «переходный этап», «зрелый этап», «возрастная витальность», «гормональная фаза».\n' +
    '• Слово «симптом» заменять на «сигнал», «маркер» или «проявление».\n' +
    '• Биомаркеры (тестостерон, эстроген, прогестерон и т.п.) упоминать только в контексте баланса и оптимизации энергии/восстановления — НЕ как «дефицит/недостаточность/гиперфункция» в смысле диагноза.\n' +
    '• Формулировать всё как оптимизацию образа жизни, энергии, восстановления, тонуса и баланса — НЕ как лечение или выявление заболеваний.\n' +
    '• В конце ОБЯЗАТЕЛЬНО добавить мягкую фразу (на ' + langName + '): результаты носят информационный характер для образа жизни, это не медицинский диагноз; при вопросах о здоровье — обсудить с врачом.\n'
  ) : '';
...
async function handleWeeklyReport(request, env, corsHeaders, ctx) {
  ...
  const weeklySystem =
    ...
    (['vio', 'pro'].includes(String(tier || '').toLowerCase()) ?
      '\n\n[WELLNESS MODE — App Store] Это велнес-, НЕ медицинское приложение. Не ставь диагнозы и не называй болезни/состояния как диагноз. Не используй «менопауза/перименопауза/андропауза» как диагноз — пиши «период гормональных перемен», «переходный этап», «возрастная витальность». Слово «симптом» → «сигнал/маркер/проявление». Биомаркеры — только в контексте баланса/оптимизации, не «дефицит как диагноз». Всё как оптимизация образа жизни и восстановления.'
      : '');
  ...
  const system = humanLed
    ...
    : (`Ты — тёплый, заботливый ИИ-советник VIA·L (направление: пременопауза, андропауза, долголетие, `+
       ...
       (['vio','pro'].includes(tier) ?
        `\n\n[WELLNESS — App Store] Это велнес-приложение. НЕ используй «менопауза/перименопауза/андропауза/климакс» как диагноз состояния — говори «период гормональных перемен», «переходный этап», «возрастная витальность», «гормональная фаза». Слово «симптом» → «сигнал/маркер/проявление». Биомаркеры (эстроген/тестостерон и т.п.) — только про баланс и оптимизацию образа жизни, НЕ как «дефицит/болезнь». Всё в велнес-рамке.`
        : ''));
