...
async function handleAnalyze(request, env, corsHeaders, ctx) {
  ...
  const wellnessGuard = isWellness ? (
    ...
    '• ⛔ Слово «кортизол» ПОД ПОЛНЫМ ЗАПРЕТОМ (даже в пользе добавок/адаптогенов) — используй «уровень стресса», «стресс-нагрузка», «реактивность на стресс». И НЕ цитируй конкретные исследования/числа из клинической базы (мета-анализы, «РКИ», проценты вроде «−27.9%», «N=…», «+57.4 нг/дл») — в велнес-режиме это лишнее и наводит на медицину. Пользу выражай КАЧЕСТВЕННО и грамотно: вместо «ашваганда снижает кортизол на 27.9% (23 РКИ)» → «ашваганда — адаптоген: многим помогает мягче реагировать на стресс и легче засыпать». НЕ используй клинический жаргон: «гематоэнцефалический барьер» → «попасть в мозг»; «нейромедиаторы» можно, но проще — «вещества хорошего настроения (серотонин и др.)».\n' +
