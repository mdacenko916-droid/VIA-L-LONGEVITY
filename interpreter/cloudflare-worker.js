...
async function handleDayPlan(request, env, corsHeaders, ctx) {
  ...
  const guard = isWellness
    ? '\n\n[WELLNESS] Без диагнозов и болезней; без гормонов-замеров (кортизол/эстроген/прогестерон/тестостерон/инсулин). СТРОГО ЗАПРЕЩЕНЫ слова «гормон», «гормональный», «гормональный фон/баланс» (даже про добавки/фитоэстрогены) → пиши «внутренний баланс», «поддержка внутреннего баланса», «переходный этап». «менопауза/андропауза» → «переходный этап»/«возрастная витальность». Не цитируй исследования/числа РКИ. Дозы — только как ориентиры.'
