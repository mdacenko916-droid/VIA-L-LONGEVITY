...
function buildUserMessage(data, lang, tier) {
  ...
  const fullDataBlock =
    ...
    + (data.today_context ? 'Контекст на сегодня (со слов клиента — ОБЯЗАТЕЛЬНО учти при интерпретации: это может объяснять «отклонения» в биометрии, например простуда → низкий SpO₂; не тревожь, если сигнал объясняется этим): ' + data.today_context + '\n' : '')
