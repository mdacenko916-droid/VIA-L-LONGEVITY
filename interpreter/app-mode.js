...
(function(){
  ...
  function apply(){
    ...
    // Юр-страницы (Конфиденциальность / Условия) в приложении открываем В ТОМ ЖЕ окне.
    // С target="_blank" в iOS-webview ссылка просто НЕ РАБОТАЛА: WKWebView не создаёт новое окно
    // для локального файла, и нажатие ничего не делало (живой iPhone 2026-09-01). На Android то же
    // самое случайно работало — там WebChromeClient грузит такую ссылку в то же окно.
    //
    // ⚠️ Ловим КЛИК, а не перебираем ссылки при старте. Первая попытка снимала target разовым
    // querySelectorAll — и не сработала: часть юр-ссылок рисуется динамически (renderCard строит
    // блок «Резервная копия», когда человек открывает профиль), то есть ПОЗЖЕ нашего прохода.
    // Делегирование на документе покрывает и уже существующие ссылки, и будущие. 2026-09-01.
    if (!window._appLegalNav) {
      window._appLegalNav = true;
      document.addEventListener('click', function (e) {
        var a = e.target && e.target.closest && e.target.closest('a[href]');
        if (!a) return;
        var href = a.getAttribute('href') || '';
        if (href.indexOf('legal/') < 0) return;
        e.preventDefault();
        location.href = href;                // относительный путь резолвится от текущей страницы
      }, true);                              // capture: раньше остальных обработчиков
    }
