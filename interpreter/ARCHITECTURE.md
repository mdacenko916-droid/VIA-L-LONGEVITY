...
### 4.9 Аккаунт по коду: синхронизация между устройствами (VIA-L EXPERT, 2026-07-31)

**Проблема.** Дневник/профиль/разборы жили только в `localStorage` устройства. PWA на телефоне и обычный браузер — **разные хранилища** (у standalone-PWA свой контейнер), поэтому один клиент с одним кодом выглядел как несколько разных, а что именно доедет нутрициологу в кабинет — было лотереей.

**Решение.** Код доступа = аккаунт. Блок функций `_sync*` в `interpreter-via-l-expert.html` (рядом с `_saveDaily`):

- **Что синхронизируется** — список `_SYNC_KEYS`: `viae_profile`, `viae_daily`, `viae_history`, `viae_steplast`, `viae_ai_memory`, `viae_ai_daily`, `viae_day_plan`, `viae_days`, `viae_lastdata`, `viae_lastanswers`, `viae_imported`, `viae_source`, `viae_spec_link`, `viae_research_consent`, кэши недельного/месячного разбора. **НЕ синхронизируются:** доступ (`viae_access/code/plan/expiry`), `viae_pid` (id установки), OAuth-sid'ы устройств (`viae_*_sid`), язык.
- **Метки изменений** — патч `localStorage.setItem` в одном месте пишет `viae_synct` = `{ключ: ts}`; «сырая» запись без меток — через сохранённый `_lsSet` (применение серверного снимка не должно выглядеть как локальная правка). Данные, накопленные до появления синка, получают метку лениво в `_syncLocalKeys()`.
- **Слияние — только на клиенте** (`_syncMerge`): обычные ключи last-write-wins по `t`; **`viae_daily` — по дням** (`_syncMergeDaily`), при конфликте дня побеждает запись с большим `ts`. Вечерний проход с телефона не стирает утренний с ноутбука.
- **Старт** (`_syncBoot`): pull → merge → отдаём управление `_bootRoute` → push своего. Вызывается в **двух** точках: `gateUnlock` (ввод кода) и `_start` (повторный заход по сохранённому коду). Сторож 4 с — сеть тупит, клиент не ждёт перед пустым экраном. Нет сети → `_syncReady=false`, **пуш запрещён** (иначе пустое устройство затрёт аккаунт), повтор через 30 с; приложение работает как раньше, локально.
- **Пуш** — дебаунс 2.5 с после последней записи + `visibilitychange:hidden` (сворачивание PWA). Конфликт `409` → pull, merge, один повтор, дальше отложенно.
- Нативный app (Capacitor) идёт мимо: там доступ по IAP, кода нет → `_syncBoot` сразу отдаёт управление.

### 4.10 Дневной разбор → кабинет (2026-07-31)

`/analyze` из `interpreter-via-l-expert.html` теперь шлёт `code` (`_syncCode()`). Раньше не слал — воркер умел ингест (`cabinetIngestIpAnalysis`, §5.х), но получал пустой код, и у специалиста были только недельные отчёты без дневной динамики. Теперь в карточку идёт полный текст разбора + срез метрик + `dyn` для «Динамики недели».
...
**R2-биндинг `ASSETS` → bucket `vial-assets`** — файлы внутреннего чата VIA-L EXPERT
(фото и голосовые). Кладёт их `handleChatUpload` (`POST /chat/upload?code=…&kind=image|audio`,
потолок 12 МБ на файл) по ключу `chat/<код карточки>/<таймстамп>-<рандом>.<ext>`;
публичный хост `https://pub-2f2f1a44a98c46beabafa751ba849852.r2.dev`, ссылка на файл
уезжает в текст сообщения в D1-карточке (`d.messages`).

> **Lifecycle: файлы чата удаляются через 365 дней.** Правило `chat-files-365d`
> (префикс `chat/`, `Expire objects after 365 days`) заведено 2026-08-30 в самом R2 —
> кода не касается, работает на стороне Cloudflare. Проверить/изменить:
> `wrangler r2 bucket lifecycle list vial-assets` / `… lifecycle add vial-assets <имя> "chat/" --expire-days N`.
> **Важно:** удаляется только объект в R2 — сообщение со ссылкой в карточке остаётся,
> поэтому во вложениях старше года картинка не откроется, а голосовое не проиграется.
> Причина срока: программы идут 8–12 недель, годовой хвост перекрывает ведение с запасом.
> Объём не проблема: бесплатный тариф R2 — 10 ГБ, на 2026-08-30 занято 2 МБ.
...
### 5.8 Состояние аккаунта `/expert/state` (2026-07-31)

Хранилище для §4.9. KV `EXPERT_DRAFTS`, ключ `ipstate:<КОД>`, формат `{rev, updated, keys:{ключ:{t,v}}}`, TTL ~400 дней, потолок снимка 3 МБ.

- `GET /expert/state?code=` → `{ok, rev, updated, keys}`; нет записи → `rev:0, keys:{}` (новый аккаунт).
- `POST /expert/state {code, baseRev, keys}` → `{ok, rev}`; `baseRev ≠ rev` → **409** `{conflict:true, rev, keys}`.

Сервер — тупое хранилище с CAS по `rev`, **логики слияния в нём нет** (вся на клиенте, §4.9): так два устройства не затирают друг друга, а воркер не дублирует правила мерджа дневника. Авторизация — код доступа, как у `/expert/message` и `/weekly-report`. Кто знает код — читает и пишет состояние: **код нельзя раздавать нескольким людям**, иначе их дневники сольются в один аккаунт.
...
- **Worker**: `/ultrahuman/start|callback|metrics` + `ultrahumanRefresh`, по образцу Oura. Auth `auth.ultrahuman.com/authorise`, token `partner.ultrahuman.com/api/partners/oauth/token` (form, креды в теле, access 24ч), scope `ring_data`. KV `ultrahuman:<sid>` (TTL 30д). `/metrics` — 7 параллельных запросов `/api/partners/v1/user_data/metrics?date=` → `data.metric_data[]{type,object}`: `avg_sleep_hrv.value`→фолбэк `hrv.avg`→hrv, `night_rhr.avg`→rhr, `Sleep.quick_metrics[type=total_sleep].value` (сек)→sleepHours, `Sleep.sleep_stages[type=deep_sleep].stage_time` (сек)→deepMin, `temperature_deviation.value`→tempDev, `recovery_index.value`→readiness, `vo2_max.value`→vo2, `Sleep.spo2.value`→spo2; каждое поле за самый свежий день. Ключи сверены 2026-09-17 по образцу ответа от поддержки Ultrahuman (`скрин/ultrah/response.jsonc`); живого кольца ещё не было (`wrangler tail`, лог `ultrahuman types`).
