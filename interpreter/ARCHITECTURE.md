...
### 4.8 Недельный AI-разбор (`weekly-report`)

Помимо подневного AI-анализа (§4.5), у клиента есть **связный разбор динамики за неделю**. Реализован одинаково на всех трёх платных страницах (pro / pro-expert / elite) — блок функций `_wrUI` / `_wrGate` / `renderWeeklyReportUI` / `genWeeklyReport`, рендерится под виджетом «Динамика недели» (`#weeklyReport`).

- **Данные:** `weeklyTrend()` (§уже существовал, считает 7-дневные средние и дельты к прошлой неделе из `vial_history`). `genWeeklyReport()` сворачивает это в `summary = {n, metrics:[{key,avg,prev,delta}]}` и шлёт на `/weekly-report`.
- **Частота — 1 раз в 7 дней.** Гейт на `localStorage`: `vial_weekly_last` (ts), `vial_weekly_text` (кэш), `vial_weekly_date`. Пока 7 дней не прошло — показывается кэш + «следующий через N дн.», кнопки нет. Смысл: только с недельным шагом видна реальная картина изменений, а не дневной шум.
- **Специалист (EXPERT/ELITE).** Запрос идёт с `code` (из `sessionStorage.vial_code`) → воркер кладёт тот же разбор в карточку кабинета как `breakdowns[type:'weekly']` (см. §5.7). PRO кода не шлёт → разбор остаётся клиентским.
- Текст разбора короткий (4–6 фраз): что улучшилось / что просело / вероятная причина / один фокус-эксперимент на неделю. Тёплый тон, образовательная рамка, без диагнозов и личных имён.
- Строки UI — во всех 12 языках (объект `_wrUI`).
...
### 5.7 Недельный разбор `/weekly-report` (`handleWeeklyReport`)

Эндпоинт под фичу §4.8. Вход: `{summary, lang, code}`. `buildWeeklyUserMessage()` разворачивает `summary` в читаемый блок (для каждой метрики: 7-дневное среднее + направление и «improved/worsened» к прошлой неделе, через карту «что есть хорошо» — для HRV/сна/энергии/QoL/готовности рост хорош, для пульса покоя/тревоги — падение). Отдельный компактный system-prompt: короткий тёплый разбор 4–6 фраз, образовательная рамка, без диагнозов и личных имён, + критичный блок языка ответа. Роутинг модели тот же, что в `/analyze` (Sonnet для he/ar/ja/ko, Haiku остальным). `max_tokens: 900`.

**Ингест к специалисту** (`cabinetIngestWeekly`): если пришёл `code` и есть D1 — в фоне (`ctx.waitUntil`) кладём разбор в карточку как `breakdowns[{type:'weekly', source:'ip', text}]`. Дедуп: запись `weekly` моложе 6 дней не дублируется, а заменяется (защита от обхода клиентского гейта сбросом localStorage). Только UPDATE существующей карточки (EXPERT/ELITE), как и `cabinetIngestIpAnalysis`. В кабинете (`cabinet/index.html`) тип `weekly` добавлен в выпадашку разборов (ключ `bd_weekly` ×12), текст раскрывается через `<details>`.
...
+ текст ответа → `faqAdd`. При новом вопросе клиента `faqMatch` — СЕМАНТИЧЕСКИЙ поиск
(1 вызов Haiku: новый вопрос + нумерованный список эталонов → индекс совпадающего по смыслу
или -1; ловит синонимы/перефраз) — ПОДСКАЗЫВАЕТ нутрициологу в топик
...
**Засев базы (2026-06-04, в проде):** `faq:db` наполнена — **409 пар** (408 из черновика
`faq-seed-draft.md` + 1 живая, сохранённая кнопкой), дедуп по вопросу. Черновик — 10 батчей
(блоки A–B7), правила вшиты в шапку: ru, гендер-нейтральность специалиста, термин «профильный
специалист» (не «нутрициолог» — ИП → платформа для разных специалистов), тон заботы (дистиллят
Perry Academy: Mintz/Sims/Boules), медицинское/рецептурное → «к врачу», без дозировок вслепую.
Залито НАПРЯМУЮ в KV: `wrangler kv key put "faq:db" --path=<json> --namespace-id=<EXPERT_DRAFTS>
--remote` (БЕЗ правки воркера и без временного роута). Конвертер `.md→json` — `/tmp/faq-seed-build.js`
(регенерит из `faq-seed-draft.md`). Вычитка специалистом — постфактум по живым диалогам (база
лишь ПОДСКАЗЫВАЕТ, человек правит перед отправкой). Опц. на будущее: префильтр в `faqMatch`
перед Haiku (сейчас шлёт все ~409 вопросов списком на каждый запрос — работает, но тяжелеет с ростом).
...
- `EXPERT_DRAFTS` (код-связка, §18.11): `code_owner:<код>`→{email,name,lang} (при покупке),
  `code_topic:<код>`→topicId (единый адрес топика клиента), `interp_email:<email>` (метка
  «клиент ИП» — гейт Cal-письма). TTL 200 дней.
...
### 18.11 Код-связка: анкета и переписка в ОДИН топик (2026-06-05, в проде)

**Проблема:** анкету клиент получает письмом (ещё до того, как открыл care-бота → топика
нет), а переписку — в Telegram (топик по chat_id). Раньше это давало ДВА топика на одного
человека; при потоке клиентов нутрициолог путается. **Решение — код доступа как мостик**:
он проходит и через письмо, и через интерпретатор.

Поток:
- При покупке ИП (`handleInterpreterPurchase`) пишем `code_owner:<код>`→{email,name,lang}
  и метку `interp_email:<email>`.
- Письмо-подтверждение Expert-заявки (`apps-script.js` `CLIENT_CONFIRM` + `ANKETA_INVITE`,
  12 яз) даёт ссылку `book/anketa/?code=<код>&lang=<lang>` — её получает **КАЖДЫЙ
  EXPERT/ELITE** (PRO до этой ветки не доходит — нет Expert-доступа). Это и есть «первое
  письмо, которое получают все».
- Кнопка «Telegram-чат» в ELITE (`#careChatBtn`) несёт `?start=<код>` (fallback — язык).
- **`code_topic:<код>` — единый «адрес» топика.** И сабмит анкеты (`deliverAnketa`), и
  `/start` care-бота резолвят/создают топик через него: **кто первый — создаёт, второй
  переиспользует**. Итог — анкета и чат в ОДНОМ топике, в любом порядке.
- Cal-письмо (`CAL_ANKETA_MAIL`, codeless `?src=elite`) клиентам ИП НЕ шлётся (гейт
  `interp_email`) — иначе ссылка без кода создала бы топик-двойник. Остаётся только для
  «несвязанных» (бронь консультации без покупки ИП).

Файлы: `apps-script.js` (CLIENT_CONFIRM/ANKETA_INVITE), `cloudflare-worker.js`
(`handleInterpreterPurchase`, `deliverAnketa`, care `/start`, `handleCalWebhook`),
`book/anketa/index.html` (читает+шлёт `?code`), `interpreter-elite.html` (`#careChatBtn`).
Проверено end-to-end (2026-06-05): ELITE (EN) + PRO+EXPERT (RU) — письмо→форма→сводка в
топик по коду; двойной сабмит одного кода → один топик (двойников нет).
...
- (Косметика) если топик создан анкетой («Имя · анкета · LANG») и потом подключается чат —
  переименовать его в «Имя · LANG» (`editForumTopic`). На работу не влияет.

---

## 19. PRO — ПЕРЕСБОРКА: «МОЯ КАРТОЧКА» + РИТМ СБОРА ДЕНЬ/НЕДЕЛЯ (2026-06, канон)

> ⚠️ Этот раздел заменяет описание PRO как «16 шагов» в §4.2 для файла
> **`interpreter-pro.html`** (vio/pro-expert/elite пока по старой схеме). Источник правды
> по структуре карточки и плану сбора — здесь, а не в памяти.

### 19.1 Идея
PRO = «гаджет есть всегда» (нет гаджета → это VIO, не PRO). После онбординга клиент попадает
не в линейную анкету, а в **личную карточку + сбор по ритму**. Фаза цикла НЕ вводится —
определяется в результате. Пересобрано 2026-06-14, ритм утверждён 2026-06-16.

### 19.2 «Моя карточка» (`renderCard`, PRO-ONLY — в pro-expert/elite/vio функции нет)
Локальный экран клиента, всё в `localStorage`. Вкладка `card` → `renderCard()`. Блоки сверху вниз:
1. **📊 Полнота данных** (`_completeness`) — индикатор покрытия, **6 категорий**: 3 с гаджета
   (HRV, Пульс, Сон) + 3 из профиля (Профиль[возраст+рост], Вес, Цикл[жен.]). Цвет по %.
   Это «сколько собрано», НЕ список метрик.
2. **⚠ Задачи** — метрики, не пришедшие с трекера (`hrv/rhr/sleepHours/tempDev`) → кнопка
   точечного ручного ввода (`_fixMetric`).
3. **Профиль** — пол, возраст, рост, вес, талия.
4. **Данные с трекера** — реальные значения с гаджета, **до 9 показателей** (см. 19.4).
   Показывается только при наличии данных (иначе `card_empty`).
5. **Мои разборы** — история сессий (score по датам).

### 19.3 Где что хранится (localStorage, PRO)
- `vial_profile` — профиль (пол/возраст/рост/вес/талия/цикл).
- `importedData` (+ зеркало `vial_imported`) — последние данные трекера (живут между перезагрузками).
- `vial_history` (≤60) — сессии-разборы: score + ключевые поля (hrv/sleep/rhr/energy/qol/mood/anxiety/phase).
- `vial_daily` (≤90, одна запись на день) — **дневной журнал** «По дням» (`_saveDaily`/`_renderDaily`):
  снимок 9 метрик + приливы. Это уже часть ЕЖЕДНЕВНОГО ритма (см. 19.5).
- `vial_lastanswers` — «как вчера» (перенос ответов прошлой сессии).
- `vial_weekly_*` — гейт и кэш недельного разбора.

### 19.4 Девять показателей трекера (канон «карточка хранит всё, что дал гаджет»)
`hrv` (мс) · `rhr` (пульс покоя) · `sleepHours` (сон, ч) · `deepMin` (глубокий, мин) ·
`tempDev` (откл. темп., °C) · `spo2` (%) · `vo2` (VO2max) · `energy` (энергия/recovery, /10) ·
`readiness` (готовность, %). Ни один гаджет не даёт все 9 (Oura ближе всех; обычные часы 1–7;
energy/readiness — фирменные баллы Oura/WHOOP). Показываются в «Данные с трекера» (renderCard),
панели «Данные с гаджета» (`renderGadgetPanel`) и дневном журнале — только пришедшие.
Обе кнопки импорта (Apple Health/Health Connect и прямой OAuth) кормят ОДИН набор этих 9 полей.

### 19.5 Ритм сбора (канон 2026-06-16) — этапы
- **Первый вход:** онбординг → **блоки + карточка** (профиль/цикл + первый импорт + первый разбор).
- **Ежедневно** (делаем после первого входа): авто-гаджет + приливы (частота/интенсивность) вручную;
  недобор метрики → сигнал-задача; одна запись в `vial_daily` на день.
- **Недельно:** полный сбор + авто + AI-разбор (`/weekly-report`, §4.8); настроение/тревога/стресс/
  самочувствие — **НЕДЕЛЬНЫЕ**, в дневную карточку первого входа не идут.
