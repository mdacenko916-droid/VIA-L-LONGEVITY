...
## 16. ЦЕЛЕВАЯ МОДЕЛЬ КАНАЛА ВЕДЕНИЯ (решение 2026-06-03, к реализации)

Проектное решение по тому, как клиент ELITE/PRO+EXPERT общается с нутрициологом
после первого PDF-разбора, как доставляется полная анкета и как это удешевить.

**Статус 2026-06-03:** ✅ **ЯДРО КАНАЛА РЕАЛИЗОВАНО И В ПРОДЕ.** Клиентский бот
`@viael_care_bot` + webhook `/care-webhook` в воркере: клиент пишет боту →
локализованное приветствие (мультиязык, deep-link `?start=<lang>`) → вопрос
создаёт топик-на-клиента в супергруппе «VIA-L Ведение» (id в секрете
`NUTRITIONIST_GROUP_ID`, бот-админ форума) + перевод на ru → нутрициолог отвечает
в топике → перевод на язык клиента → доставка в личку. Кнопка ELITE
(`#telegramBlock`) переведена на бота с языком из ИП. Протестировано end-to-end.
Секреты `CLIENT_BOT_TOKEN`, `NUTRITIONIST_GROUP_ID`. KV `care_topic:`/`care_client:`/
`care_lang:` (без TTL). Деплой версия `fa00753e`+.
**Осталось (TODO):** цифровизация полной анкеты (110 Q × 12 яз + ИИ-структурирование),
FAQ-память «эталон», расширение словаря фраз, Cal-webhook → ссылка на анкету.
