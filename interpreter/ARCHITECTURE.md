...
FAQ-память «эталон» (KV + кнопка backstage), Cal.com — строку со ссылкой
на анкету в шаблон подтверждения Zoom (настройка в Cal, не код).
...
- Полная локализация email-подтверждения на 12 языков.
- Авто-выдача ссылки на анкету сразу после покупки/перехода в ведение (сейчас ссылку
  выдаёт команда `/anketa` нутрициолога вручную).
