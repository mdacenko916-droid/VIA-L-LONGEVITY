...
- **Данные:** `weeklyTrend()` (считает 7-дневные средние и дельты к прошлой неделе из `vial_history` — 7 числовых метрик). **+ `_weeklyDaily()` (2026-06-19):** агрегирует `vial_daily` за 7 дней — то, чего нет в истории: приливы (дней с приливами + модальная частота/интенсивность), `tempDev`/`spo2`/`stress`/`memory` средние, дни тумана/алкоголя; плюс разовый контекст из `vial_profile` (фаза/ПМС/диета/режим питания). `genWeeklyReport()` шлёт `{summary, daily, lang, code}` (раньше — без `daily`). PRO-only пока (сбор `vial_daily` живёт в pro).
...
Эндпоинт под фичу §4.8. Вход: `{summary, daily, lang, code}` (`daily` — с 2026-06-19, опционально). `buildWeeklyUserMessage(summary, daily, lang)` разворачивает `summary` в читаемый блок (для каждой метрики: 7-дневное среднее + направление и «improved/worsened» к прошлой неделе, через карту «что есть хорошо» — для HRV/сна/энергии/QoL/готовности рост хорош, для пульса покоя/тревоги — падение). **Если пришёл `daily`** — дописывает блоки «Daily detail logged this week» (приливы/температура/SpO₂/память/туман/стресс/алкоголь) и «Client context» (фаза/ПМС/диета/режим питания); строка в `weeklySystem` велит ИИ опираться на них. Без `daily` (старый клиент / EXPERT-кабинет) сообщение идентично прежнему — обратно совместимо. Отдельный компактный system-prompt: короткий тёплый разбор 4–6 фраз, образовательная рамка, без диагнозов и личных имён, + критичный блок языка ответа. Роутинг модели тот же, что в `/analyze` (Sonnet для he/ar/ja/ko, Haiku остальным). `max_tokens: 900`.
