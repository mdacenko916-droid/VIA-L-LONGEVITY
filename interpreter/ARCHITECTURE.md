...
#### Путь 3 — EXPERT (ИП, PRO+EXPERT): первый PDF → email с анкетой → care-группа
1. EXPERT покупается в ИП — `prog_email` НЕ ставится; канала care/Cal у тарифа нет
   (только email + 2 PDF в окне 30 дней). Короткая onb_-анкета (§3.2) приходит с первым
   Expert-запросом и даёт нутрициологу стартовый контекст; полная 110-Q анкета —
   отдельно, по триггеру ниже.
2. **Триггер.** Нутрициолог отправляет первый PDF через backstage-бота (callback
   `approve:` → `send_report` успешно). В ветке `sentOk` воркер проверяет
   `draft.plan==='EXPERT'` **и** `sendResult.reports_sent_total===1` (только ПЕРВЫЙ
   отчёт; dev-коды дают 0 и не триггерят) → шлёт клиенту письмо `buildExpertAnketaMail(lang)`
   (EN-led, ссылка **`book/anketa/?src=expert&lang=<lang>`**). Карточка успеха в backstage
   получает строку «📋 Анкета (110 Q) отправлена клиенту».
3. **Заполнение.** Клиент открывает ссылку (есть `src` → `SEND=true`) → `POST /anketa-submit`
   с `src=expert` (без `intake`/`topic`) → care-путь (`deliverAnketa`): ИИ-сводка + полные
   ответы в **care-группу** новым топиком по имени+email + письмо-подтверждение `ANKETA_MAIL`.
   Приёмная часть та же, что у ELITE (Путь 2) — отдельного кода не потребовала.
   *Зачем после первого PDF:* анкета даёт нутрициологу контекст для второго разбора в окне.
