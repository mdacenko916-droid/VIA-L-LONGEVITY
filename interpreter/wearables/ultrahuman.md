...
**Краткий статус (2026-09-17):** ✅ **разбор метрик сверен и исправлен по образцу от Ultrahuman.**
- 12.09 поддержка (Manasa) починила 403 на метриках → проверено: 200, но пусто — к аккаунту владельца не
  привязано кольцо. 16.09 запросили демо-данные, 17.09 Sandeep прислал образец ответа
  (`скрин/ultrah/response.jsonc`, список в `data.metric_data`).
- На образце наш разбор терял 4 из 8 полей: сон, глубокий сон, отклонение температуры, SpO2 (искали
  `sleep.*.minutes`, а на деле `Sleep` с большой буквы, секунды в `quick_metrics`/`sleep_stages`,
  `temperature_deviation.value` отдельным типом, `Sleep.spo2.value` одним числом). Исправлено, прогон
  функции воркера на образце даёт все 8, задеплоено 2026-09-17.
- ✅ `_nativeHideUltrahuman` снят 2026-09-17 (`060174a`): Android v6 в закрытом тесте, iOS 1.0.2 через Xcode Cloud. ⏳ Ответить Sandeep
  с результатом. Живого кольца по-прежнему нет — первый реальный клиент проверить по `wrangler tail`.

**Статус на 2026-09-10 (история):** ✅ **ключи получены самостоятельно, код написан** — ⏳ ждёт
...
- 🟡 **Живая проверка 2026-09-10 (владелец, аккаунт БЕЗ кольца):** вход и экран согласия «VIA-L wants to
  access your Ultrahuman account — Ring Data» ✅, обмен кода ✅ (токен в KV `ultrahuman:<sid>`),
  `…/oauth/token/info` → 200 (наш application uid, scope `ring_data`). НО `…/user_data/metrics` и
  `user_info` → **403 с пустым телом** (без `Bearer` → 401 invalid_token, т.е. токен они принимают, а данные
  не отдают). Воркер отвечает `fetch_failed` → в карточке «Не удалось получить данные». Не различено:
  403 = «у аккаунта нет кольца» или «приложению не включён доступ к данным (whitelist)». Решится первым
  владельцем кольца или ответом Ultrahuman.
- 📨 **2026-09-10 отправлено письмо в `support@ultrahuman.com`** с `integration@via-l.com` (текст —
  `docs/ULTRAHUMAN-API-403-LETTER.md`): 403 — это «нет кольца» или нужно включить доступ, и есть ли тестовые
  данные. ⏳ Ждём ответа. (Первая попытка отбилась: в Gmail «Отправлять как» устарел пароль приложения.)
- Фронт (оба тарифа): в карточке только «🔗 Подключить Ultrahuman» (старый ручной блок кольца убран 2026-09-10,
  ручной ввод — кнопка «Ввод данных с трекера»), карточка в «Синхронизации устройства» сразу под Oura, общий
